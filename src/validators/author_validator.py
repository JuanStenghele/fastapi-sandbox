...
class AuthorValidator():
  ...
  def validate_deletion(self, session: Session, author_ids: list) -> None:
    ...
    if authors_ids_not_found:
      raise ValidationError(detail = f"AUTHORS_NOT_FOUND: {[str(id) for id in authors_ids_not_found]}")

    author_ids_with_books = self.author_dal.get_author_ids_with_books(session, author_ids)
    if len(author_ids_with_books) != 0:
      raise ValidationError(detail = f"AUTHORS_HAVE_BOOKS: {[str(id) for id in author_ids_with_books]}")
